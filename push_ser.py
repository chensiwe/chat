...
import threading
import redis
red=redis.Redis(host='127.0.0.1',port=6379,decode_responses=True,db='2')
user=set()
...
def handler_msg(conn):
        ...
        with conn as c:
            while True:
                ...
                print('yuanshi.........',data_recv)
                if data_recv[0:1] == b"\x81":
                    data_parse = parse_payload(data_recv)
                    print(data_parse)
                if re.match("^admin.*",data_parse):
                    
                    #print(connlist)  
                    uss=copy.copy(user) 
                    for cli in uss:
                        print('current....',cli,type(cli))
                        send_msg(cli, bytes("admin say: {}".format(data_parse), encoding="utf-8"))
                else:
                    send_msg(c, bytes("client say: {}".format(data_parse), encoding="utf-8"))
